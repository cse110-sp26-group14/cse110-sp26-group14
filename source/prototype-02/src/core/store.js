...
export class Store {
  ...
  /** Get weekly slot keys for a user (When2Meet style). */
  getWeeklySlots(userId) {
    return this.state.weeklySlots[userId] || [];
  }

  /** Persist weekly slot keys for a user. */
  setWeeklySlots(userId, slots) {
    this.state.weeklySlots[userId] = slots;
    this.save();
  }
  ...
  getAvailabilityLogs() {
    return this.state.availabilityLogs;
  }
  ...
  submitWeeklyAvailabilityCheck(payload) {
    ...
    const weekKey =
      payload.weekKey || getISOWeekKey(payload.submittedFor || new Date());
    ...
    const log = {
      ...
      userId,
      userName:
        this.getUsers().find((user) => user.id === userId)?.name || "Unknown",
