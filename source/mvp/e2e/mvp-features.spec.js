...
test.describe("MVP features — sprint, reports, meetings", () => {
  ...
  test("settings can add a new sprint to the list", async ({ page }) => {
    ...
    await page
      .locator("#sprint-add-form")
      .getByRole("button", { name: "Add sprint" })
      .click();
    await expect(page.locator(".sprint-list")).toContainText("Sprint E2E");
    await expect(
      page.locator("#header-sprint-select option:checked"),
    ).toContainText("Sprint E2E");
  });
  ...
  test("header sprint selector switches dashboard sprint", async ({ page }) => {
    ...
    await expect(page.locator("#app")).toContainText(
      /Real-time overview of Sprint 3/,
      { timeout: 10000 },
    );
  });

  test("check-in notes field saves and appears on Reports tab", async ({
    page,
  }) => {
    ...
    await page
      .locator("#checkin-form")
      .getByRole("button", { name: "Submit Check-In" })
      .click();
    ...
    await expect(page.locator(".reports-list")).toContainText(
      "E2E progress with notes",
    );
  });
  ...
  test("issues Activity tab shows unified timeline", async ({ page }) => {
    ...
    await expect(
      page.locator(".activity-list .activity-card").first(),
    ).toBeVisible();
  });
  ...
  test("availability can schedule meeting on calendar", async ({ page }) => {
    ...
    await page.locator("#avail-add-meeting-btn").waitFor({ state: "visible" });
    await page.locator("#avail-add-meeting-btn").click();
    ...
    await page
      .locator("#meeting-form")
      .getByRole("button", { name: /add to sprint calendar/i })
      .click();
    await expect(page.locator("#modal-host")).toHaveClass(/hidden/);
  ...
  test("calendar shows sprint deadline chip on sprint end date", async ({
    page,
  }) => {
    await openHash(page, "#calendar");
