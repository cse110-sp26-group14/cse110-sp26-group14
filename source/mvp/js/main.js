...
import {
  generateTeamSummaryRemote,
  suggestSprintTasksRemote,
...
async function startApp(authUser) {
  ...
  try {
    ...
    if (useRemoteData()) {
      ...
      showToast(
        "Could not load team data. Check API URL and Worker deploy.",
        "error",
        6000,
      );
    }
...
function wireAvailability() {
  bindOnce(document.getElementById("btn-availability"), "click", () => {
    ...
    const existing =
      store.getState().availability?.[date]?.[uid] ||
      store.getUsers().find((u) => Number(u.id) === Number(uid))
        ?.availability ||
      {};
    ...
    bindModalForm(
      ...
      async (formData) => {
        ...
        await saveAvailabilityRemote(
          store,
          formData.get("date") || date,
          slots,
        );
  ...
  window.addEventListener("sitrep:open-task-modal", () => {
    ...
    bindModalForm(
      ...
      async (formData) => {
        ...
        const sprintId =
          Number(formData.get("sprintId")) ||
          store.getSelectedSprint()?.id ||
          2;
        ...
        if (assignees.length >= 2 && task?.id) {
          ...
          showToast(
            `Task created with ${assignees.length} sub-tasks, one per assignee.`,
            "success",
            4000,
          );
        }
      ...
      },
      {
        ...
        successToast: (assignees) =>
          assignees?.length >= 2 ? false : "Task added to sprint.",
      },
    );
  });
}
...
function wireTaskDetail() {
  ...
  window.addEventListener("sitrep:open-task-detail", (e) => {
    ...
    const subtasks = store
      .getState()
      .tasks.filter((t) => Number(t.parentTaskId) === Number(taskId));
          ...
          <div style="display:flex;flex-direction:column;gap:0.4rem;">
            ${subtasks
              .map((sub) => {
                ...
                const subA = sub.assignees?.length
                  ? sub.assignees
                  : sub.owner
                    ? [sub.owner]
                    : [];
...
function wireSubtaskEvents() {
  ...
  window.addEventListener("sitrep:split-task", async (e) => {
    ...
    if (assignees.length < 2) {
      showToast(
        "Task needs 2+ assignees to split into sub-tasks.",
        "warning",
        4000,
      );
      return;
...
function wireMeetingModal() {
  ...
  window.addEventListener("sitrep:open-meeting-modal", (e) => {
    ...
    const hasRecommendation = Boolean(detail.teamCount && detail.teamSize);
    const recommendationBanner = hasRecommendation
      ...
      async (formData) => {
        ...
        if (formData.get("createTask")) {
          await createTaskRemote(store, {
            title: formData.get("title"),
            priority:
              formData.get("severity") === "critical" ||
              formData.get("severity") === "high"
                ? "high"
                : "medium",
            owner: assignee,
            due,
...
async function runAiTaskSuggestions(goals, aiBtn) {
  ...
  await runWithButtonFeedback(
    ...
    async () => {
      ...
      modal.show(
        "Review AI task suggestions",
        AiTaskReviewForm(suggestions, store),
      );
      const form = document.getElementById("ai-task-review-form");
      document.getElementById("ai-review-reject-all")?.addEventListener(
        "click",
        () => {
          modal.close();
      ...
      form?.addEventListener("submit", async (ev) => {
          ...
          .map((_, i) =>
            fd.get(`pick-${i}`)
              ? {
                  ...
                  owner:
                    fd.get(`owner-${i}`) || authUser?.name || "Team Member",
                  due: fd.get(`due-${i}`),
                  sprintId,
                  status: "open",
                }
        ...
        try {
          ...
          if (result.log?.id) {
            await updateAiLogStatusRemote(
              store,
              result.log.id,
              picked.length ? "applied" : "rejected",
            );
          }
          if (useRemoteData()) await refreshStoreFromApi(store);
          showToast(
...
function wireAiActions() {
  ...
  bindOnce(aiBtn, "click", () => {
    ...
    document
      .getElementById("ai-goals-cancel")
