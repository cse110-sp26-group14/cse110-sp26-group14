...
export class IssuesView extends BaseView {
    ...
    const reports = this.store
      ...
      .filter(
        (r) =>
          sprintId == null ||
          r.sprintId == null ||
          Number(r.sprintId) === Number(sprintId),
      )
      .sort((a, b) =>
    ...
    return reports
      .map((r) => {
        const user = this.store
          .getUsers()
          .find((u) => Number(u.id) === Number(r.userId));
  ...
  renderActivityPanel() {
    const items = buildActivityTimeline(
      this.store,
      this.store.getSelectedSprint()?.id,
    );
    if (!items.length) {
    ...
    return items
      .slice(0, 40)
      .map((item) => this.renderActivityCard(item))
      .join("");
  }
  ...
  renderTasksPanel() {
    ...
    const tasks = this.store
      .getState()
      .tasks.filter(
        (t) => sprintId == null || Number(t.sprintId) === Number(sprintId),
      )
      .sort(
        (a, b) =>
          ...
          <div class="inline-card-actions">
            ...
            ${
              issue.status !== "resolved"
                ...
                : `<button type="button" class="action-btn unresolve-issue-btn" data-issue-id="${issue.id}">Re-open issue</button>`
            }
          </div>
        </div>
  ...
  renderTaskCard(task) {
    ...
    const aiTag =
      (task.tags || []).includes("AI Suggested") || task.source === "ai"
  ...
  renderActivityCard(item) {
    if (item.sourceKind === "issue") {
      const issue = this.store
        .getIssues()
        .find((i) => Number(i.id) === Number(item.sourceId));
      return issue ? this.renderIssueCard(issue) : "";
    }
    if (item.sourceKind === "task") {
      const task = this.store
        .getState()
        .tasks.find((t) => Number(t.id) === Number(item.sourceId));
      return task ? this.renderTaskCard(task) : "";
    }
    if (item.sourceKind === "report") {
      const report = this.store
        .getReports()
        .find((r) => Number(r.id) === Number(item.sourceId));
      const user = this.store
        .getUsers()
        .find((u) => Number(u.id) === Number(report?.userId));
      return report
        ? this.renderReportCard(report, user?.name || "Member")
        : "";
    }
    if (item.sourceKind === "ai") {
      const log = this.store
        .getAiLogs()
        .find((l) => Number(l.id) === Number(item.sourceId));
  ...
  render() {
    ...
    const tabs = [
      ...
      .map((t) => renderTabButton({ ...t, active: this.panel === t.id }))
      .join("");
  ...
  mount(container) {
    ...
    container.querySelectorAll(".unresolve-issue-btn").forEach((btn) => {
      btn.addEventListener("click", async () => {
        const id = Number(btn.dataset.issueId);
        await unresolveIssueRemote(this.store, id);
        container.innerHTML = this.render();
        this.mount(container);
      });
    });
    ...
    container.querySelectorAll(".inline-edit").forEach((field) => {
      const eventName = field.matches('select, input[type="date"]')
        ? "change"
        : "blur";
      field.addEventListener(eventName, () =>
        this.saveInlineEdit(field, container),
      );
    });
  }
  ...
  async saveInlineEdit(field, container) {
    ...
    try {
      ...
      if (kind === "issue")
        await updateIssueRemote(this.store, numericId, patch);
      if (kind === "task")
        await updateInlineTaskRemote(this.store, numericId, patch);
      if (kind === "report")
        await updateReportRemote(this.store, numericId, patch);
      if (kind === "ai") await updateAiLogRemote(this.store, numericId, patch);
      field.dataset.original = value;
      const freshStatus = container.querySelector(
        `[data-save-status="${kind}:${id}"]`,
      );
      if (freshStatus) {
        freshStatus.textContent = "Saved";
        window.setTimeout(() => {
          const laterStatus = container.querySelector(
            `[data-save-status="${kind}:${id}"]`,
          );
          if (laterStatus?.textContent === "Saved")
